...
void User_Systick_T::start(void)
{
    ...
    for(m_times = 1; LOAD_value > 0xFFFFFF; m_times *= 2)
        LOAD_value /= 2;
    m_tmp = m_times;
...
/**
  * @brief  use this function to timing at place like waiting for initialization
  * @param  ms waiting time
  * @retval None
  */
void User_Systick_T::timing(uint32_t ms)
{
